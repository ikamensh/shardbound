...
def branch(payload, cls, replace=False, turns=4):
...
def recovery_input(mode):
    """Lose an actually played second shard, then select its surviving retinue."""
    state = play_stage(State.new_campaign(7, 'Commander', difficulty=mode))
...
def recovery_probe(payload, cls, policy):
    state = cls.from_json(json.dumps(payload))
    events = [snapshot(state)]
...
def scarcity_probe(payload, cls, *, earn_first):
    state = cls.from_json(json.dumps(payload))
    events = [snapshot(state)]
    if earn_first:
        state.explore()
...
def main():
    ...
    sources = sorted([*ROOT.joinpath('eador').glob('*.py'), Path(__file__).resolve(),
                      *(ROOT / 'tools' / name for name in ('prototype_eador_late_realm.py', 'audit_eador_difficulty.py',
                      'audit_eador_economy.py', 'eador_linked_campaign.py', 'eador_campaign.py', 'stress_eador_control.py'))])
    ...
    for name, cls in VARIANTS.items():
        ...
        for replace in (False, True):
            events = branch(examples['late_full_roster']['state'], cls, replace)
            assert events == branch(examples['late_full_roster']['state'], cls, replace)
            late[name]['replace_same_role' if replace else 'keep'] = events
        pursuit[name] = branch(examples['pursuit_last_action']['state'], cls, True, turns=1)
    recovery_inputs = {mode: recovery_input(mode) for mode in ('standard', 'challenge')}
    for mode, payload in recovery_inputs.items():
        for name, cls in VARIANTS.items():
            for policy in ('wait_three', 'shrine_first'):
                row = recovery_probe(payload, cls, policy)
                assert row == recovery_probe(payload, cls, policy)
                recovery[f'{mode}/{name}/{policy}'] = row
    scarcity = {}
    for name, cls in VARIANTS.items():
        for earn_first in (False, True):
            row = scarcity_probe(recovery_inputs['challenge'], cls, earn_first=earn_first)
            assert row == scarcity_probe(recovery_inputs['challenge'], cls, earn_first=earn_first)
            scarcity[f'{name}/{earn_first}'] = row
    report = dict(revision=subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
                  source_sha256=hashes, examples_sha256=hashlib.sha256(examples_path.read_bytes()).hexdigest(),
