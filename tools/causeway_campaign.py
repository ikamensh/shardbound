...
def prepare_causeway(hero_class='Commander', *, seed=7, difficulty='standard', mana=0, state=None, budget=None):
    ...
    if state.hero.hero_class == 'Scout':
        prepare_explorer('Scout', support=None, state=state, budget=budget)
    else:
        prepare_adventure(theme='ruins', state=state, budget=budget)
...
def causeway_retry_route(state, *, orders_type=AdventureOrders, budget=None):
