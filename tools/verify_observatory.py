...
from tools.observatory_campaign import prepare_observatory, observatory_route, observatory_rune_route
...
def verify(output, *, backend='pyglet', approach='clear', support='sapper'):
    ...
    sources = sorted([*ROOT.joinpath('eador').glob('*.py'), *framework_sources(),
                      *[ROOT / 'tools' / name for name in ('campaign.py', 'roles_campaign.py',
                          'extraction_campaign.py', 'control_campaign.py', 'observatory_campaign.py',
                          'audit_extraction.py', 'ui.py', 'verify_control.py',
                          'verify_extraction.py', 'verify_observatory.py')]])
