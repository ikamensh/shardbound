...
def measure(*, budget=None):
    ...
    for mode in ('accessible', 'standard', 'challenge'):
        for seed in (0, 2, 7, 11, 29):
            for hero, routes in (
                ...
                paid = prepare_relief(state=Purchases(State.new(seed, hero, difficulty=mode)), budget=budget)
                ...
                for name, route in routes:
                    ...
                    plans[f'{key}/{name}'] = {**report, 'settled': settle_once(play, budget=budget)}
    baseline = State.from_json(json.dumps(parties['standard/7/Commander']['snapshot']))
    failed = relief_failed_support(baseline, orders_type=record_orders)
    ...
    retry = relief_retry_route(replacement, orders_type=record_orders)
