...
def _rest(state):
    ...
    rest(state, defend=state.rival.intent == 'attack' and state.rival.target is not None
         and state.grid.distance(state.rival.target, (-2, 0)) <= 1)


def prepare_relief(hero_class='Commander', *, seed=7, difficulty='standard', state=None):
    ...
    for destination in ((-1, -1), (-1, 0)):
        march_to(state, destination); _rest(state)
