...
desktop fullscreen, and reduced motion. A window-size choice leaves fullscreen.
...
These controls do not implement text scaling or establish that all game screens
meet G10's readability/accessibility matrix.

Sound, display and motion are game-owned preferences in
...
restores entry audio, motion, fullscreen and the actual window size remembered
...
`eador.app.create_game` loads sound/motion and restores saved display once at
...
load audio and reduced motion. This prevents a hidden verifier or packaging smoke
...
`load_preferences(game)` applies sound/motion only, so title entry, campaign load
...
text scaling, cross-platform display support or Steam release readiness.
