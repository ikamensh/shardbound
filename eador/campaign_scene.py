...
class CampaignScene(Screen):
    ...
    def depart(self):
        ...
        if not self.checkpoint(state):
            self.root.message = self.message + ' The departure snapshot is retained in autosaves.'
        self.game.pop()
    ...
    def draw(self):
        ...
        if self.step == 'offers':
            ...
            for column, label in ((0, f'VETERANS · {len(self.troop_ids)}/2 KEPT'), (1, f'RELICS · {len(self.relic_ids)}/2 KEPT')):
                ...
                if len(items) > 6:
                    self.text(f'{self.cursors[column] // 6 + 1} / {(len(items) + 5) // 6} · Up/Down reaches every item',
            ...
            if items:
                ...
                self.paragraph(detail, x + 36, y + 501, width=1008, size=12, color=TEAL)
